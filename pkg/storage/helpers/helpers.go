...
// CheckConnection checks if the storage is accessible
func CheckConnection(storage types.Storage, timeout time.Duration) error {
	// Create a context with given timeout
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()

	// Try to list objects in the bucket with a timeout
